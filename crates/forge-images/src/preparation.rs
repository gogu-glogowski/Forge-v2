...
mod tests {
    ...
    #[test]
    fn old_broker_fields_are_ignored_and_active_state_is_preserved() {
        let fixture = Fixture::new();
        let preparation = durable_preparation(fixture.plan());
        let mut old_json = serde_json::to_value(&preparation).unwrap();
        let object = old_json.as_object_mut().unwrap();
        object.insert(
            "helper_bootstrap".to_owned(),
            serde_json::json!({"helper_sha256": "old-helper"}),
        );
        object.insert(
            "preparation_channel".to_owned(),
            serde_json::json!({"channel_name": "org.majorforge.preparation.0"}),
        );
        object.insert(
            "read_only_guest_inventory".to_owned(),
            serde_json::json!({"operation_id": "old-operation"}),
        );
        object.insert(
            "privileged_offline_discovery".to_owned(),
            serde_json::json!({"broker_version": "old-broker"}),
        );
        ...
        let decoded: FedoraWorkstationPreparation = serde_json::from_value(old_json).unwrap();
        assert_eq!(decoded.preparation_id, preparation.preparation_id);
        assert_eq!(decoded.staging.path, preparation.staging.path);
        assert_eq!(decoded.status, preparation.status);
        assert!(!decoded.qemu_img_check_attested);
        assert_eq!(decoded.execution, preparation.execution);
    }
